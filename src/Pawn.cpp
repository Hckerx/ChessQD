...
void Pawn::findMovesWithoutCheck(std::vector<std::shared_ptr<Piece>>& Pieces){
        ...
        if(getMatchingPiece(glm::vec2{pos.x, pos.y-step}, Pieces) == nullptr && !(pos.x > 7 || pos.x < 0 || pos.y-step > 7|| pos.y-step < 0)) {
                legalMoves.push_back(glm::vec2(pos.x,pos.y - step));
                if (((pos.y == 6 && white) || pos.y == 1) && getMatchingPiece(glm::vec2{pos.x,pos.y - 2*step}, Pieces) == nullptr ){
...
bool Pawn::move(glm::vec2 newPos, glm::vec2 oldPos, std::vector<std::shared_ptr<Piece>>& Pieces, bool whiteTurn, bool isPlayingOnline, bool isWhite) {
